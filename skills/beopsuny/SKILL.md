...
- 수정 조항 요청에는 검토용 완성 초안과 수정 이유·전제·확인 사항을 제공한다. 법률적 보증이나 실제 외부 행동 권한을 뜻하지 않으며, 최종 법률 판단은 사용자가 한다. 초안은 결론·초벌이므로 위 Output contract gate(`references/output-formats.md`)가 붙는다.
