...
| `policies/source_grades.yaml` | 소스 등급 판단 모호할 때 | Grade A/B/C/D 정책 + 기본 매핑 |
...
Citation의 분자/분모는 응답에 등장한 1차 소스(법령·판례·행정규칙) 인용 개수. 인용이 0개인 답변(절차 안내, 용어 설명 등)은 `Citation n/a`로 표기. Dim 2/3는 pass(✓) / fail(⚠) 이분법으로만 표시한다.
