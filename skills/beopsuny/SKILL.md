...
user question
-> 개인정보 쟁점이 실질적인지 판단
-> 필요하면 taxonomy로 issue/fact question만 넓게 framing
-> 법순이 live search 수행
-> 필요하면 retrieval hints로 누락 검색어/source family 확장
-> 필요하면 authority map으로 post-search audit
-> 현재 live source와 Source Grade 기준으로 답변
```

경계:

- 현재 structured ingestion 대상은 `privacy` active vertical만이다.
...
- 통지·신고 판단표에 **현행법 결론과 시행예정 전환 리스크**가 분리되어 있는지 — 시행예정 법령(2026.9.11 개정법 등)을 현행법 기준으로 오답변하지 않도록
...
| `mandatory_provisions.yaml` | Dim 4 서브체크 1 판정 시 | 한국 강행규정 단일 소스 (약관·IP·개인정보·공정거래·근로) |
...
| `knowledge-injection.md` | beopsuny-knowledge manifest 소비 경계와 privacy recall/audit 보강 규칙 |
...
3. 위험도 **상** 조항 대표 예시:
   ...
   - Data Processing → 개인정보보호법 제26조 (위탁) · 제28조의8 (국외이전, 2023-09-15 시행)
...
- [ ] `alt_wording_hint` 방향이 한국 강행규정 하에서 **유효 가능한** 범위인가? (명백 위반 문구 제안 없음). 강행규정 단일 소스는 `assets/policies/mandatory_provisions.yaml` — 조항 유형별 `clause_types` 매칭으로 적용 조문 조회
