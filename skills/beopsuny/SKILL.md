...
5. **Counter-drafting 힌트 출력** (조항 엔트리에 `why_risky` 등 필드가 있을 때).
   조항별 출력 블록에 `review_mode` 별로 아래 3 필드를 필터링해 제시한다:

   | 모드 | `why_risky` | `negotiation_points` | `alt_wording_hint` |
   |------|-------------|----------------------|---------------------|
   | strict   | ✓ | ✓ | ✓ |
   | moderate | ✓ | ✓ | — |
   | loose    | ✓ | — | — |

   - `negotiation_points` 는 `profile.yaml` 의 당사자 위치(갑/을)에 맞춰 `gap`/`eul` 중 관련 관점을 우선 노출. 위치가 불명이면 양쪽 모두 표시
   - **경계**: 출력은 반드시 *힌트(방향·원칙)* 로 표현. "아래 문구로 교체하세요", "이 문구를 사용하세요" 같은 **단정적 자동 생성 뉘앙스 금지** (`references/contract_review_guide.md:23` 원칙). 최종 문구 확정은 사용자 몫
