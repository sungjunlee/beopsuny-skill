...
5. **Counter-drafting 힌트 출력** (조항 엔트리에 `why_risky` 등 필드가 있을 때).
   ...
   - `negotiation_points` 는 `profile.yaml` 의 당사자 위치(갑/을)에 맞춰 `gap`/`eul` 중 관련 관점을 우선 노출. `profile.yaml` 에 당사자 위치 필드가 없거나 값이 불명이면 **양쪽(gap·eul) 모두 노출** 이 기본값 — v0.2.x 에서는 스키마에 해당 필드가 없으므로 사실상 항상 양쪽 노출
