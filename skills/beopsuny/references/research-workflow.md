...
Full 모드에서 `admrule-kr`가 있으면 행정규칙 원문 후보를 먼저 읽는다. `본문출처: parsing-failed`이거나 첨부파일이 결론에 필요하면 law.go.kr 원문 또는 첨부 파일을 추가 확인한다.
...
Full 모드에서 `ordinance-kr`가 있고 지역이 특정되면 자치법규 원문 후보를 먼저 읽는다 (전역 검색은 비용이 크므로 지자체·지역을 먼저 좁힌다). 지역이 없으면 결론을 만들지 말고 확인 질문 또는 범위 가정을 표시한다.
