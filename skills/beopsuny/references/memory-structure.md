...
회사 사실은 `profile.yaml` 전체의 단일 출처이고, 계약 표준 입장은 기존 `profile.yaml.contract_playbook`의 단일 출처로 유지한다. 새 `practices/contract.yaml`은 출력 선호·쟁점 순서·escalation·반복 질문·external counsel handoff 같은 allowed_scope overlay만 담는다.
...
다만 요청에 playbook 설정 의도(예: "계약 검토 playbook을 맞춰줘")가 포함되거나 특정 practice 영역(contract/privacy/labor/regulatory/litigation)을 지목해 해당 영역 playbook 또는 practice profile 설정 의도를 보이면 full 온보딩으로 즉시 전환한다. 모호한 경우에는 quick 시작 전에 "계약 playbook 또는 업무별 practice profile 항목까지 함께 설정할까요?"라고 확인한다.
...
quick 항목에 아래 계약 검토 playbook 항목 또는 선택된 practice profile 항목을 추가한다. full 온보딩은 단순 설문이 아니라 evidence-based onboarding이다. 사용자가 제공한 seed document, 과거 검토 이력, playbook 문서가 있으면 실제 체결 조건과 stated position의 차이를 추출해 저장 전 확인한다.
...
full 온보딩 내 선택 단계에서 사용자가 특정 practice 영역을 지목하거나 해당 영역 playbook 설정 의도를 보이면 practice profile 단계를 추가한다. 이 단계는 `practice_profile.yaml`의 allowed_scope 항목인 출력 선호, `issue_sequence`, `escalation_thresholds`, `repeated_questions`, `external_counsel_handoff`를 인터뷰와 seed 추출 후보로만 채운다.
...
이 절차는 계약 playbook에만 한정하지 않는 seed document 후보 추출의 단일 절차다. stated position과 signed practice의 차이는 추출 항목의 성격에 따라 목적지를 나눈다. seed document에서 추출한 회사 사실은 항상 `~/.beopsuny/profile.yaml`로 라우팅한다. 업무별 출력 선호·쟁점 순서·escalation·반복 질문은 해당 `~/.beopsuny/practices/<area>.yaml` 후보로 라우팅한다.
...
- output_preferences 후보: 사용자가 표시한 목적지, 선호 섹션, reviewer note style
- issue_sequence 후보: 반복적으로 먼저 확인하는 쟁점 순서
- escalation_thresholds와 repeated_questions 후보
- external_counsel_handoff 후보: 전달 형식, source ledger 포함 여부, business context 포함 여부
...
| 소스 조회 provenance | 프로젝트별 (`source_log.jsonl`) | 어떤 source를 열었는지는 프로젝트 맥락에 종속 |
...
| 산출물 보관 | 프로젝트별 (`outputs/`) | 리포트·메모·redline 힌트는 프로젝트 맥락에 종속 |
| 활동 인덱스 | 프로젝트별 (`history.jsonl`) | 시간순 포인터와 요약은 프로젝트 맥락에 종속 |
...
각 엔트리는 append-only source/provenance 기록이다. 결론, 사용자 1차 확인, 법령 권위 판단은 여기 쓰지 않는다.
...
`source_type`은 `law|precedent|regulation|api|file|url|other`, `action`은 `consulted|pulled` 중 하나를 사용한다.
