...
- 기존 번들 자산과 knowledge asset이 충돌하면 현재 live official source와 출처 권위 라벨을 우선하고, 오래된 번들 문구는 확인 필요로 둔다.
...
- SKILL.md 라우팅 원칙 1(Right-sizing)에 해당하는 좁은 질문 — 특정 조문·시행일·공식 링크 확인, live search stack만으로 충분한 경우
...
- 모든 결론은 출처 권위 라벨과 verification tag를 붙인다.
