...
> **경로 해석 주의**: `~` 해석이 실행 컨텍스트마다 달라질 수 있다(예: Hermes 프로필 샌드박스의 터미널 HOME과 에이전트 컨텍스트의 `~`가 다른 경로를 가리킬 수 있음). 미러를 설치한 뒤에도 세션이 바뀌면 기본값 `~/.beopsuny`를 못 찾을 수 있다. 미러를 실제 사용하는 프로필·환경에서는 `BEOPSUNY_DATA_ROOT`를 **절대 경로로 고정**하는 것을 권장한다 — 예: 프로필 `.env`(`~/.hermes/profiles/<프로필>/.env`)에 `BEOPSUNY_DATA_ROOT=/absolute/path/.beopsuny`를 추가. `~` 의존 없이 동작하는 것이 검증 기준이다.
...
**미러는 읽기 전용 git 데이터다.** `legalize-kr`·`precedent-kr`·`admrule-kr`은 upstream(GitHub)에서 `git pull`로 갱신되는 공식 원문 스냅샷이므로 **파일을 직접 편집·수정·추가하지 않는다.** 조회·읽기만 하고, 갱신이 필요하면 아래 `## 데이터 초기화`의 동기화 절차(pull --ff-only)를 쓴다.

### 법령 파일 선택 (현행본 판별)

법령 디렉토리에는 여러 파일이 있을 수 있다: `법률.md`, `법률(법률).md`, `시행령.md`, `시행규칙.md`. **`법률.md`가 항상 현행 통합본인 것은 아니다** — 대부분 법령은 `법률.md`가 현행 통합본이지만, 일부 법령(예: 근로기준법)은 `법률.md`가 옛 폐지 법률의 조항만 담은 스텁(630B, "근로기준법은 이를 폐지한다" 조항뿐)이고 현행 통합본은 별도 파일이다.

선택 순서:

1. `법률(법률).md`가 있으면 **그것을 현행 통합본으로 우선**한다(미러 전역 21건에서 확인된 패턴).
2. `법률(법률).md`가 없으면 `법률.md`를 열어 frontmatter(`공포일자`, `시행일자`, `상태`)와 파일 크기·본문 구조로 현행 여부를 판별한다. 본문이 스텁(폐지 조항·부칙뿐, 1KB 미만)이면 같은 디렉토리의 다른 파일이 있는지 확인하고, 현행 원문을 찾지 못하면 law.go.kr로 확인한다.
...
조문 제목·본문뿐 아니라 **벌칙·과태료 조항(예: 제110조, 제116조)의 호(号) 리스트, 별표, 수치·금액·기한도 시행 전후로 바뀔 수 있다.** 형사벌칙·과태료·금액·기한이 판단을 좌우하는 답변에서는 미러 파일만으로 결론을 확정하지 말고 **law.go.kr 현행본으로 대조**한다. 실전 예: 근로기준법 미러 파일(공포 2026-06-09, 시행일자 2027-06-10의 시행 예정 개정분)은 제110조 제1호·제116조 제2항 제2호의 위반 조항 리스트가 현행(law.go.kr, 시행 2025.10.23)과 달랐다 — 벌칙 조문 번호뿐 아니라 "어느 조항 위반이 어느 벌칙/과태료 대상인지" 자체가 미반영 개정 때문에 drift한다.

## 법망 API

무인증 무료 API다. 상세 엔드포인트는 `references/beopmang-api.md`를 읽는다.

**비공식 서비스다.** 법망 API는 공식 기관 운영이 아니라 개인 재능기부로 운영되는 서버다. 가용성·신뢰성에 기대하지 않는다 — 2026-08 기준 장기 중단 상태이며 복구 시점도 보장되지 않는다. 로컬 미러·law.go.kr·korean-law-mcp가 부재할 때의 보조 경로로만 쓰고, 중단 시 남은 공식 경로로 좁힌다.
...
| 대상 | 경로 | 비고 |
| --- | --- | --- |
| 법제처 법령해석 | `opinion.lawmaking.go.kr` (통합법령정보센터 법령해석 검색) | 공식. 외부 추출 도구가 타임아웃될 수 있어 브라우저로 시도. 예: 법제처 21-0913(2022.4.26) — '시간외근로' 기준은 법정근로시간 |
| LAWnB 유권해석 | `lawnb.com` 유권해석 DB | 상용 법률DB. 원문 재확인 보조 |
| 고용노동부 행정해석 | 고용노동부 누리집 정책자료·행정해석 (moel.go.kr) | 예: 여성고용정책과-3874(2020.10.12) — 임신기 단축근무 초과는 제74조제7항 위반(노무법인 뉴스레터 재인용) |
...
주의: LSW `lsInfoP.do` URL은 해당 법령 **전체 본문**을 반환한다(조문 하나만이 아님). 외부 추출 도구로 열면 잘릴 수 있는데, 이때 전체 본문은 추출 도구의 캐시 파일로 저장되므로 read_file로 페이지네이션해서 조문을 찾는다. `lsiSeq`는 현행 법령 버전마다 다르므로, 아무 조문이나 한 번 열어 화면에 노출된 lsiSeq 값을 사용한다.
...
초기화 전에 `BEOPSUNY_DATA_ROOT`를 절대 경로로 설정해 둔다(아래 `## 소스 가용성 확인`의 경로 해석 주의 참고). 프로필 단위 환경 변수(예: Hermes 프로필 `.env`)에 값을 고정해 두면 이후 세션·컨텍스트가 달라져도 미러를 다시 찾을 수 있다.
