...
수정 조항을 요청하면 편집 가능한 완성 초안을 작성한다. 주요 조항의 강행규정·예외가 적용되는 주체·행위·요건과 공포·시행·사건 적용 시점 및 경과규정을 공식 원문에서 확인한다. 요약과 초안에서도 적용 요건을 유지하며, 거래 유형이 미확정이면 그 분기를 남긴다. 법정 요건과 이를 구현하기 위해 제안한 추가 절차·책임 배분을 구별한다. 제안 조항도 당사자에게 직접 적용되는 법정 의무를 다른 당사자의 승인에 종속시키거나 제한해서는 안 된다. 사용자의 협상 위치 및 확인된 사실을 적용한다. 완성 초안과 수정 제안은 검토용이며, 최종 법률 판단은 사용자가 한다.
