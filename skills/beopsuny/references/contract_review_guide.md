...
계약 검토는 계약 문구를 최종 확정하는 기능이 아니라, 조항별 한국법 이슈와 확인해야 할 근거를 좁히는 workflow다. 공통 출처 권위 라벨, verification status, 자가 검증, 면책 고지는 `SKILL.md`, `references/source-grading.md`, `references/self-verification.md`, `references/output-formats.md`를 따른다.

## 범위

지원: 한국어·영문 계약 조항의 한국법 이슈 식별, 계약 유형·당사자 위치·거래 구조에 따른 위험 조항 분류, 강행규정 충돌 가능성·누락 조항·협상 포인트 제시, 영문 법률용어의 한국법상 뉘앙스 설명.

경계:

- 위험도는 `상/중/하` 실무 우선순위이지 자동 점수나 확정 평가가 아니다.
- Counter-drafting은 확정 문구가 아니라 검토 힌트와 협상 방향만 제공한다. "아래 문구로 교체", "최종 수정안", "이 문구를 사용" 같은 단정 표현은 금지한다.
...
- 명확한 단문 질문에는 관련 조항과 핵심 근거만 답한다. 사용자가 "전체 검토", "엄격하게", "전수"를 요청하거나 장문 계약서를 제공한 경우에만 full review로 확장한다.
- 법률 문제와 회사 협상 선호를 같은 결론처럼 섞지 않는다. 결론을 좌우하는 사실이 없으면 질문하거나 가정을 표시한다.

## Destination

산출물이 어디로 갈지에 따라 메타정보와 문체를 조절한다. 목적지가 명시되지 않으면 내부 법무 검토용을 기본으로 보되, 상대방 송부·기관 제출·현업 전체 공유처럼 보이면 확인한다.

| 목적지 | 처리 |
| --- | --- |
| 내부 법무 검토용 | 검토자 메모, 출처 권위 라벨, 자가 검증, 미확인 범위를 모두 표시 |
...
| 기관/외부 제출용 | 제출 전 변호사 또는 담당 법무 검토 gate를 명시. 확인되지 않은 법령·사실은 포함하지 않음 |
...
모드는 `strict`("엄격하게", "꼼꼼히", "전수 검토") / `moderate`(기본값) / `loose`("간단히", "핵심만", "요약") 3종이다. 각 모드의 동작, 출력 필드(`why_risky`, `negotiation_points`, `alt_wording_hint`) 출력 여부, 금지 문구는 `assets/policies/review_mode.yaml`이 단일 소스다. 이 파일이 SKILL.md보다 우선한다.
...
**playbook**: [적용/미설정] | **escalation**: [없음/필요]
...
| 조항 | 위험도 | 이슈 | 근거 |
| --- | --- | --- | --- |
| Limitation of Liability | 상 | 고의·중과실 면책까지 포함되면 무효 가능성 | **[공식 원문] [VERIFIED]** 약관규제법 제7조 |
...
허용: 조항의 위험 이유(`why_risky`), 협상 포인트(`negotiation_points`), 확정 문구가 아니라 검토 힌트인 대안 방향(`alt_wording_hint`).

금지(경계): 최종 수정안처럼 확정된 문구 제시, 사용자에게 그대로 삽입하라는 지시, 강행규정 충돌 가능성을 확인하지 않은 대체 문언.
