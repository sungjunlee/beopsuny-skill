...
판례를 결론 근거로 쓸 때는 사건번호·선고일 확인에 그치지 않고 현재 사안과 비교한다. 비교 필드는 판례 provenance 라벨과 별개이며, source_authority나 verification status를 대체하지 않는다.
...
- **사건명**: 손해배상(기)
- **판시사항**: ...
- **법원 원문**: https://www.law.go.kr/LSW/precInfoP.do?precSeq={판례일련번호} (로컬 미러 frontmatter `출처` 우선)
- **법령정보**: https://www.law.go.kr/판례/({사건번호})
- **유사점**: 현재 사안과 공통되는 법률요건, 사실관계, 쟁점
- **차이점**: 현재 사안과 다른 사실관계 또는 쟁점
- **적용 한계**: 사실 전제, 쟁점 범위, 하급심 여부, 원문 확인 범위 등 이 판례 적용 시의 한계
- **distinguishing**: 차이로 인해 결론이 달라질 수 있는지. 차이가 중요하면 결론 강도를 낮추고 추가 확인 항목으로 둠
```
...
- **유사점**: ...
- **차이점**: ...
- **적용 한계**: 하급심 변경 가능성과 현재 사안과의 사실 차이
- **distinguishing**: 차이로 인해 결론이 달라질 수 있는지
```

law.go.kr 판례 원문을 직접 열어 확인한 경우에는 다음처럼 표시한다.
...
밀도(draft-first 초벌 품질): 로컬 미러가 있어 인용이 실제로 `[VERIFIED]`인 근거는 자신 있게 단정하고, 과잉 `[UNVERIFIED]`·중복 유보로 초벌을 흐리지 않는다. 유보 태그는 실제로 확인하지 못한 항목에만 붙이고, 검토자 메모와 `Before relying`는 변호사가 의존 전 확인할 1-2개로 짧게 유지한다. 이는 밀도 조절일 뿐 verification status·출처 권위 라벨 계약을 완화하지 않는다 — 확인 못 한 것을 확인한 것처럼 쓰지 않는다.
...
role이 미지정·미확정인 채로 destination이 지정되거나 `legal_effect_triggers`에 해당하면, 둘 중 하나만 고르지 않고 `unknown`/`business_user` gate와 destination 계약을 함께 합성한다. 합성 조건과 해소 원칙(더 엄격한 쪽 우선, must_strip 합집합, must_include 양쪽 모두, 서명·송부·제출 직접 지시 금지 유지)은 스키마의 `composition_rule`을 단일 소스로 따르고 여기서 다시 나열하지 않는다.
...
| `external_draft` | 외부 공유용 초안임을 표시하고, 보내기 전 법무 검토 필요 문구를 포함 | 내부 검토자 메모와 자가 검증 블록, 그리고 다른 건·상대방·협상 조건을 식별하는 사실을 그대로 포함하지 않는다 |
| `agency_or_court_submission` | 제출용 문안이 아니라 확인된 사실·근거·누락 항목 정리 중심 | 변호사 또는 담당 법무 검토 없이 제출하라고 쓰지 않음. 다른 건·상대방·협상 조건을 식별하는 사실도 포함하지 않음 |
...
같은 이유로 다른 건에서 온 사실도 대외 산출물에 싣지 않는다. 한 작업 디렉터리가 여러 건을 담는 것이 기본이므로(SKILL.md `## 회사 맥락`), 읽어온 맥락에 다른 건의 상대방명·협상된 조건·거래금액이 섞여 있을 수 있다. 현재 건에 필요한 사실만 남기고, 다른 건을 식별하는 사실은 근거로도 예시로도 쓰지 않는다.
...
자가 검증 블록 표기의 단일 소스다. 인용이 0개인 절차 안내, 용어 설명 등은 `Citation n/a`로 표기한다.
