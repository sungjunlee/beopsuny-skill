...
계약 문서에서 온 값은 quote와 location을 붙인다. 법령/행정규칙/판례에서 온 값은 출처 권위 라벨과 verification status를 붙인다. 후보 체크리스트나 clause mapping만으로는 `answered`가 될 수 없다. `answered`는 quote/location 또는 live 출처 권위 verification이 있을 때만 허용한다.
...
표 안의 법률 리스크 셀은 stale 후보 데이터를 그대로 `answered`로 승격하지 않는다. 특히 아래 column type 또는 내용은 `references/source-access.md#freshness-gate`와 `references/freshness-governance.md`를 적용한다.
