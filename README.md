...
```bash
BEOPSUNY_INSTALLED_SKILL_PATH=~/.agents/skills/beopsuny python tests/validate_skill_contracts.py
```
...
| 품질 계약 | 기준 문서 | 회귀 검증 |
| --- | --- | --- |
| Legal verification core | `skills/beopsuny/references/research-workflow.md#legal-verification-core`, `skills/beopsuny/assets/schemas/legal_verification_packet.yaml`, `skills/beopsuny/references/self-verification.md`, `skills/beopsuny/references/source-grading.md` | `tests/validate_skill_contracts.py`, `tests/scenarios/16_router_regression.yaml` `router-16` |
| Source Grade / VERIFIED 계약 | `skills/beopsuny/references/source-grading.md`, `skills/beopsuny/assets/policies/source_grades.yaml` | `check_source_grading_verified_contract`, `legal_status_tag`, `no_verified_uncertainty` |
| Freshness governance | `skills/beopsuny/references/freshness-governance.md`, `skills/beopsuny/assets/policies/freshness_debt.yaml`, `skills/beopsuny/assets/schemas/freshness_revalidation.yaml`, `skills/beopsuny/references/source-access.md#freshness-gate` | `check_freshness_debt_registry`, `router-15` |
...
8. `python -m pip install -r requirements-dev.txt`, `python tests/validate_skill_contracts.py`, `python tests/evaluate_scenario_outputs.py`, `python -m py_compile tests/validate_skill_contracts.py tests/evaluate_scenario_outputs.py`, `git diff --check`를 실행한다.
...
### ④ 메모리/검증 스키마 — `assets/schemas/` (9 files, Full 모드 전용)
