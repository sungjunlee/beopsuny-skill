...
### 2026-09-12 사용자 정정: 변호사 업무와 지식 축적·컴파일

주된 활용자는 변호사인 배우자다. 그 업무를 돕고 모델 발전에도 정확히 재사용할 의미 있는 지식을 쌓는 것이 목적이다. 실사용과 전문가 피드백은 지식의 가치·누락·적용 경계를 확인하는 경로이며, 가족 일반 도구의 가치 검증이나 피벗이 주목적은 아니다.

- 앞선 가족 일반 사용 3건/피벗 트랙 #341은 계획 철회로 종료했다. 그 해석에 따라 바꾼 #323의 AC5도 원문으로 복원했다. #323/#317은 실제 출력 회귀가 남아 OPEN이며, 근거 대조 단계 실험은 향후 후보일 뿐 미실행이다.
- #316은 하위 #319/#321/#322/#325와 통합 판단 #326의 완료 근거에 따라 종료를 유지한다. 출시 또는 모델 품질 PASS를 뜻하지 않는다. 당장 출시하지 않는 상황에서 미래 외부 제공 검토 #340은 보류 기록으로 유지한다.
- 장기 지식 방향과 실제 한 건의 누락 원인 분류는 [knowledge #79](https://github.com/sungjunlee/beopsuny-knowledge/issues/79)에 연결했다. 지식은 출처·적용 조건·예외·반례·확인 시점·재확인 지점·도출 이유를 보존하며, 공급 시 이를 잃지 않고 필요한 소비 지점에 전달해야 한다. 모델별 출력 개선과 지식의 재사용 가치는 별도로 검증한다.
- knowledge의 research_hold는 새 운영 주입·대량 축적·영역 확대의 보류이며 장기 지식 축적 목표 자체의 폐기가 아니다. 근거 보존·오류 교정은 계속하고 실제 누락과 직접 근거에서 최소 후보를 검증한다. 이번 정정으로 runtime·평가 원본·지식 자산을 바꾸거나 새 모델을 실행하지 않는다.
