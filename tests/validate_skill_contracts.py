...
_TESTS_DIR = str(ROOT / "tests")
if _TESTS_DIR not in sys.path:
    sys.path.insert(0, _TESTS_DIR)

from contract_checks.freshness import (  # noqa: E402  # #335
...
def defined_check_functions() -> set[str]:
    names: set[str] = set()
    sources = [ROOT / "tests/validate_skill_contracts.py"]
    checks_dir = ROOT / "tests/contract_checks"
    if checks_dir.is_dir():
        sources.extend(sorted(path for path in checks_dir.rglob("*.py") if path.is_file()))
    for source in sources:
        names.update(re.findall(r"^def (check_[a-zA-Z0-9_]+)\(", source.read_text(encoding="utf-8"), flags=re.M))
    return names
