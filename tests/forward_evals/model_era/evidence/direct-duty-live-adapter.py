...
runtime='/tmp/beopsuny-release09/public-runtime-direct-duty'
...
a=['timeout','-k','10s','900','claude','-p','--safe-mode','--restricted','--strict-mcp-config','--permission-mode','dontAsk','--model',e['BEOPSUNY_EVAL_MODEL'],'--effort','high','--tools','Read,Glob,Grep,WebFetch,WebSearch','--allowedTools','Read,Glob,Grep,WebFetch,WebSearch','--add-dir',runtime,data,'--system-prompt',c,'--output-format','stream-json','--verbose']
start=time.monotonic()
with (t/f'{ident}.stdout').open('w') as o,(t/f'{ident}.stderr').open('w') as z:q=subprocess.run(a,input=p,text=True,cwd=w,env=e,stdout=o,stderr=z)
final=None
