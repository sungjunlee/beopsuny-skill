...
### Fixed
- **법망 API 중단 반영 — 죽은 경로를 "기본 검색 경로"로 안내하지 않는다 (#268)** — 법망이 서버 장애로 서비스를 중단했다(복구 예상 2027-Q1). 법령·판례는 로컬 미러로 그대로 동작하고, 미러 없는 family의 discovery만 law.go.kr·korean-law-mcp·웹검색으로 내려간다. README에 현재 상태를, 스킬 문서에는 날짜 없이 가용성 조건부 서술을 둔다. 조회 실패 판정은 벤더 오류 코드 열거가 아니라 `ok: false`·5xx·timeout·빈 응답 구조로 바꿨다 — 코드명이 바뀐 순간 문서 4곳이 동시에 빗나갔기 때문이다.
