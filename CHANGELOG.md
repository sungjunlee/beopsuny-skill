...
- `tests/evaluate_scenario_outputs.py` — `business_user_external_gate` 공통 규칙의 내부 블록 누출 검사를 `자가 검증:` 단일 패턴에서 `EXTERNAL_DRAFT_INTERNAL_LEAK_PATTERNS`(검토자 메모/자가 검증/내부 사고 과정/미확인 내부 메모, markdown bold 허용)로 확장. router-14(비법무 사용자 외부 송부 초안, 텍스트 경로) 재사용 — #188의 `artifact_deployment_shared_assumption_gate`는 Artifact/URL 배포 맥락에만 적용되어 순수 텍스트 초안 경로는 못 잡던 gap을 신규 rule 없이 해소 (#107)
