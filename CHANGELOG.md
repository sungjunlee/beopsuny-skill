...
- 개인정보 지식 자산은 상한 내 전문만 전달하고 초과 자산은 전체 생략한다. 전달 상태·해시·생략 이유를 남기며 운영과 평가가 같은 기본 상한을 사용한다 (#328).

- 검토용 완성 조항 초안과 수정 제안을 지원한다. 표제 금지와 mode별 산출물 차단을 제거하고, 법률 근거·전제·확인 사항 및 실제 외부 행동 권한을 구별한다 (#318, #323, #324).
- 출력은 요청에 맞춰 구성하고, 단순 금액·기한 확인에 중복 내부 양식을 요구하지 않는다. 감사·인계용 evidence packet은 근거를 한 번 기록하는 선택형으로 정리했다 (#322, #272).
