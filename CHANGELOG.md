...
### Fixed
- **router-19 output_eval 계약 배선이 완료된다** — `check_counter_draft_output_contract_wiring` validator가 router-19(수정 조항 요청)의 `output_eval.required_substrings`가 비어 있다고 실패했다. Counter-drafting + output_contract spine 계약에 맞춰 required_substrings(수정안·수정 이유·전제·확인 사항·고의·중과실·[UNVERIFIED]·최종 법률 판단은 사용자) 7종과 forbidden_substrings(법적으로 유효하며·상대방에게 발송했습니다·최종 법률 판단을 확정) 3종을 추가했다. 사용자 관점 — PRs #344와 #348의 contracts CI 차단이 해제된다.
