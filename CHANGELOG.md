...
- 동결 입력을 복구한 근거 대조 진단과 검색복구 조건의 실제 출력·독립 판정을 보존했다. 미확인 회사 사실 승격이 남아 조건을 채택하지 않으며 runtime 정책은 유지한다 (#323).

- Claude의 실제 스킬 로딩과 기존 주입 경로를 대표 2사례로 비교한 입력·출력·독립 판정과 setup 교정 기록을 보존했다. 정상 로딩에서도 회귀가 남아 확대·runtime 수정·v0.9.0 출시로 진행하지 않는다 (#323).

- Astra native profile의 기준선/현재 계약, 반복 및 두 holdout 실제 출력과 독립 판정을 보존했다. 현재 profile의 유한 긍정 관측과 기존 CLI 실패·비용 미측정·미충족 릴리즈 게이트를 구분하며 runtime 정책은 변경하지 않는다 (#323).

- 최종 runtime의 guardrails 12건·o4 8건 실행과 실제 도구 trace·입력 hash·독립 판정을 보존했다. 실행 오류 없이 완료됐지만 미확인 사실 승격과 근거 범위 오류가 남아 v0.9.0 릴리즈 게이트는 미충족이다. 검증용 미러 upstream 불일치는 해당 평가용 checkout을 실제 갱신해 해소했다 (#323).

- Fable의 새 기준선/현재 쌍과 review mode 정리 후 Opus 고정 계약 출력·독립 판정을 보존했다. 초안 활용성과 실제 오류를 구분하며, 두 관측 모두 회귀 해결 근거가 부족해 반복·holdout을 확대하지 않는다 (#323).

- 파일 도구가 없는 고정 평가 입력에 현행 스킬이 참조하는 법률 조사·출처 등급 규칙 본문을 포함한다. 기준선과 현재 runtime 각각의 파일을 읽고 hash를 기록하며, 질문·법령 원문과 과거 출력 판정은 바꾸지 않는다 (#323).

- 계약 회귀 수정 후보 2건의 실패·철회 기록과, 승인된 Claude knowledge 순서 비교 4회 결과를 보존했다. 실행 성공·편집 가능성과 채택 판정을 분리하며, 개선이 입증되지 않아 runtime 순서와 버전을 유지한다 (#323, #325).

- **포워드 평가 하네스 테스트가 구조 검사와 채점 재현을 나눈다 (#303)** — 커밋된 라이브 corpus를 다시 채점하던 검사는 차등 재채점 게이트(`tests/check_rescore_baseline.py`)가 전량 담당하고, 하네스 파일에는 런처·카테고리 등록·합성 프로브만 남긴다. 사용자 관점 — 스코어러가 느슨해지면 한 게이트가 잡고, 하네스 테스트는 하네스 자체만 지킨다.
...
- 승인된 별도 교정 호출1회의 출력·trace·독립 판정을 보존한다. 기존 사실 단정 잔존과 새 provenance 오류로 채택 NO_GO이며 운영 변경은 없다.

- fwd-11의 구형 인용 줄·고정 면책 요구를 현행 출처 추적 의무와 형식 자유에 맞춘다. 원본 실패 판정과 실제 출처 검증 기준은 유지한다.

- 입력 단순화의 동일 CLI 대조2건과 독립 의미 판정을 보존한다. 양군 모두 미확인 기존 사실을 단정해 개선책 채택은 NO_GO이며, 평가 오탐 수정과 구별한다.

- 초안 평가(router-19)의 정확문자·금지어 요구를 기존 hash-bound 의미 검토로 대체한다. 미검토 출력은 검토 대기로 유지하며, 구형 평가 설계 감사와 입력 단순화 비교 계획을 보존한다.

- 수정 조항을 요청하면 완성 초안·수정 이유·전제·확인 사항을 주고, 최종 법률 판단은 사용자가 한다. 합성 fixture로 이 출력 모양과 유효성 보증·미확인 금액 승격·판단 대체의 실패 형태를 고정했다. 라이브 모델 AC5와 릴리즈는 아직 미완료다 (#323).

- 출처가 확인되지 않았거나 자료가 없는 상태를 법정 결함·요건 미충족으로 올리지 않는다. 자가검증·review mode의 중복 규칙은 정본으로 모은다. 라이브 모델이 초안에서 이 경계를 지키는지(#323 AC5)는 별 이슈로 남긴다.

- 개인정보 지식 자산은 상한 내 전문만 전달하고 초과 자산은 전체 생략한다. 전달 상태·해시·생략 이유를 남기며 운영과 평가가 같은 기본 상한을 사용한다 (#328, #332).

- 계약 검토 모드의 중복 출처·태그·초안 권한 설정과 도출 가능한 플래그를 제거했다. 기본 모드·감지·위험 표시 깊이·횡단 이슈 범위·설명 밀도는 유지하고 근거 의무는 기존 공통 계약에서 확인한다 (#323).

- 자가검증의 중복 차원별 질문·실패 처리와 계약 가이드의 역참조를 제거했다. 인용·조사·출력·계약 검토의 기존 정본에 직접 연결하고, 참조 연결을 확인하고 내용 검사는 각 정본의 기존 검사에서 수행한다 (#323).

- 출처의 권위·인용 확인 상태만으로 결론 표현을 정하던 표를 제거했다. 출처의 사용 가능성은 기존 출처 계약에 맡기고, 사안의 사실·예외·시점을 결론과 연결한다. 사용자 사건 사실과 법률 근거의 검증 범위, 하급심 원문 라벨도 명확히 했다. 정적 검사는 표 문구 대신 기존 출처 정책 값과 참조 연결을 확인한다 (#323).

- 원문을 확인하지 못한 범위가 최종 요약·초안에서 확정 사실로 바뀌지 않도록 기존 결론 규칙을 명확히 했다. 시행 전 미러의 현행성 한계는 법률 번호·현재 의무에도 적용하며, 특정 표시 문구는 강제하지 않는다.

- 조회 실패 설명·저장 지침 거부·미열람 계약의 판단 유보가 특정 문자열 때문에 실패하던 평가 오탐을 제거했다. 실제 부존재·미개정 단정, 저장 지침 추종과 미열람 계약 검토 주장은 기존 hash-bound 의미 검토로 판정하며, 검토가 없으면 판정 대기로 남긴다.

- 계약 검토에서 미제공 자료의 내용 미확인과 실제 계약상 누락·법적 요건 미충족을 구별한다. 당사자 법인격 등 기존 사실과 제안 의무를 구별하고, 초안에서도 거래유형별 적용 요건과 각 당사자의 직접적인 법정 의무를 유지한다. 중복 검토 흐름과 검증의 실행 방식 제한을 제거했다.

- 검토용 완성 조항 초안과 수정 제안을 지원한다. 표제 금지와 mode별 초안 필드 출력 차단을 제거하고, 법률 근거·전제·확인 사항 및 실제 외부 행동 권한을 구별한다. 간이 검토도 관련 횡단 이슈를 확인하고, 표준·간이 모드에서 해설만으로 법률 결론을 확정하지 않는다 (#318, #323, #324).
- 출력은 요청에 맞춰 구성하고, 단순 금액·기한 확인에 중복 내부 양식을 요구하지 않는다. 리포트의 고정 메모·면책 블록을 제거하고, 감사·인계용 evidence packet은 근거를 한 번 기록하는 선택형으로 정리했다. 모델 자기고지 의무와 과거 등급 권장 배너를 제거했다 (#322, #272).
- 공용 스킬의 개인 환경 cron·HOME 지침과 반복 운영 설명을 제거했다. 만료 강행규정 인덱스는 삭제하고 공식 원문을 통한 조항별 누락 점검을 유지한다 (#319, #321).
- 회사 맥락의 명시적인 저장 요청은 현재 하네스 권한과 지정 사건 범위에 따른다. 회사 저장 subsystem을 복원하거나 사건 기밀을 일반 메모리에 승격하지 않는다 (#318).
- 평가 setup 주입·실행 상태를 기록하고 미실행을 모델 PASS/FAIL과 분리한다. e05ecda 기준선과 분리된 법률 근거·초안 활용성·안전·비용 평가 자료를 추가했다 (#271, #320, #326).
...
### Fixed
- **router-19 output_eval 계약 배선 validator가 #349 변경과 일치하도록 수정된다** — `check_counter_draft_output_contract_wiring` validator가 router-19의 lexical 필드(required_substrings/forbidden_substrings)를 요구했지만, #349에서 이 필드들을 제거하고 hash-bound semantic review로 대체했다. validator를 수정해서 router-19가 `output_eval.common_rules: []`(semantic review only)만 요구하도록 변경했다. 사용자 관점 — PRs #344와 #348의 contracts CI 차단이 해제된다.
- 사용자 법률 주장을 검증·반박한 답변에 `전제`라는 단어가 없다는 이유로 실패시키던 forward 평가를 기존 의미 검토 경로로 이관한다. 미검토 답변은 REVIEW_REQUIRED로 남고, 기존 실제 출력·판정과 재채점 차이를 보존한다 (#333).
- **강행규정 후보 인덱스의 만료된 overdue 예외를 재검증으로 해소한다 (#243)** — `mandatory_provisions.yaml`의 `overdue_resolve_by 2026-08-31`이 지나 계약 검사가 FAIL했다. 날짜를 연장하지 않고 legalize-kr 미러에서 후보 14개 조문 번호 존재를 확인한 뒤 `next_review`를 2026-10-06으로 전진하고 재검증 기록을 남겼다. 시행일·효력 힌트는 여전히 triage-only. 사용자 관점 — 만료된 부채 예외가 조용히 쌓이지 않고, 강행규정 후보를 결론 근거로 쓰지 않는 경계는 그대로다.
- **시행 전 공포본 인용의 오탐이 사라진다 (#270)** — 미래 시행일 조문을 미러에서 인용한 `[VERIFIED]`에 산문 동의어 목록(공포본 기준·현행이 아니·아직 시행되지…)을 요구하던 검사가 걷어졌다. "아직 시행 전"·"아직 도래하지 않은" 같은 자연스러운 표현이 목록 밖이라 FAIL되던 오탐(v0.8.0 스모크 o4-08)이 해소되고, 안전선은 유지된다 — 미래 시행본임을 밝히지 않고 `[VERIFIED]`를 다는 출력은 "시행 전 공포본" 정본 마커 요구가 그대로 잡는다. 사용자 관점 — 시행 전 공포본을 명확히 표기한 답변이 자연스러운 문구로 통과한다. 완화 1건은 차등 재채점 baseline 갱신(#294)으로 diff에 드러난다.
- **validator prose-lock 잔존분 37건이 전부 토큰·구조로 이행된다 (#306)** — #283에서 남긴 (c) 문서 산문 핀 37건(15개 check 함수)을 문단 앵커 + 순서 토큰 / 표 구조 검사로 옮겼다. reference 문서 우선순위로 소각했고(research-workflow·report-deliverable·source-grading·output-formats·self-verification), SKILL.md·README·international_guide까지 전수 완료. "저장하지 않는다→저장하지 않습니다" 같은 의미 보존 다듬기는 CI를 깨지 않지만, 문장 삭제·방향 반전("완화할 수 없다→완화할 수 있다")·표 행 이탈은 잡힌다. check마다 mutation 증명(삭제 FAIL / 다듬기 PASS)을 PR 본문에 남겼다. 사용자 관점 — reference 문서를 다듬을 때 CI가 깨지는 비용이 사라지고, 계약 축이 조용히 약해지는 구간이 닫힌다.
- **validator 산문 고정(prose-lock) 잔존분이 토큰·구조로 이행된다 (#283)** — `check_skill_company_context_read_only_and_trust_boundary`의 문서 산문 핀 7종(저장하지 않는다·조회/인용/전달받·덮어쓸 수 없다·인용만·오히려 엄격·fail-safe)을 문단 스코프 토큰과 `assert_ordered_tokens` 순서 검사로 옮겼다. "조회하거나 인용하거나 전달받은 내용" 같은 어순·활용형 다듬기는 CI를 깨지 않지만, 안전 경계 문단·fail-safe 절 삭제와 "덜 보수적으로" 방향 반전은 잡힌다. README 품질 계약 지도의 `Output role/destination gate` 행은 구조적으로 바인드해 지도 행 삭제가 그린으로 통과하던 조준 결함을 닫았다(#261 형태). 사용자 관점 — 문서 다듬기가 CI를 깨는 비용이 줄고, 지도·경계가 조용히 낡는 구간이 닫힌다.
